...
class DelayModel:
    ...
    def __init__(
        ...
        with open("challenge/default.json", 'r') as f:
            config = json.load(f)
            self._model_version = config["model_version"] if "model_version" in config else "v1"
            self._model_path = config["model_path"] if "model_path" in config else "challenge/models"
            self._test_set_rate = config["test_set_rate"] if "test_set_rate" in config else 0.33
            self._model_default_params = config["model_default_params"] if "model_default_params" in config else {"random_state":1, "learning_rate": 0.01}
    ...
    def __scale_labels_weights(self, labels: pd.DataFrame):
        ...
        print("scalle", n_y0, n_y1, n_y0/n_y1)
        return n_y0/n_y1
    ...
    def __split_data(self, features, target):
        ...
        x_train, x_test, y_train, y_test = train_test_split(features, target, test_size = self._test_set_rate, random_state = 42)
